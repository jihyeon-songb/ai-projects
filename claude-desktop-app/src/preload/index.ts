...
const api = {
  /** Claude turn 완료 알림 구독 */
  onTurnComplete(cb: (p: NotifyPayload) => void): () => void {
    const l = (_e: unknown, p: NotifyPayload): void => cb(p)
    ipcRenderer.on('claude:turn-complete', l)
    return () => ipcRenderer.removeListener('claude:turn-complete', l)
  },
  /** 권한 요청 구독 */
  ...
  setInteractive(interactive: boolean): void {
    ipcRenderer.send('claudie:set-interactive', interactive)
  },
  /** 캐릭터를 잡고 드래그 시작 (메인이 커서를 따라 윈도우를 이동) */
  dragStart(): void {
    ipcRenderer.send('claudie:drag-start')
  },
  /** 드래그 종료 */
  dragEnd(): void {
    ipcRenderer.send('claudie:drag-end')
  },
  /** 우클릭 컨텍스트 메뉴(끄기 등) 표시 */
  showMenu(): void {
    ipcRenderer.send('claudie:context-menu')
  }
}

contextBridge.exposeInMainWorld('claudie', api)

export type ClaudieApi = typeof api

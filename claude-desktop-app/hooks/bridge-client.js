'use strict'
// 클로디 브릿지 디스커버리 + HTTP 호출 헬퍼 (의존성 없음)
const http = require('node:http')
const fs = require('node:fs')
const os = require('node:os')
const path = require('node:path')

const DISCOVERY_PATH = path.join(os.homedir(), '.claude', 'claudie-bridge.json')
...
function post(pathname, payload, timeoutMs) {
  return new Promise((resolve, reject) => {
    const info = discover()
    if (!info) return reject(new Error('claudie bridge not running'))
    ...
    if (timeoutMs && timeoutMs > 0) {
      req.setTimeout(timeoutMs, () => req.destroy(new Error('claudie bridge timeout')))
    }
    req.write(data)
    req.end()
  })

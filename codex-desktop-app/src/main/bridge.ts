...
export const DISCOVERY_PATH = join(homedir(), '.codex', 'orangi-bridge.json')
...
/** PreToolUse 훅이 stdin으로 받은 페이로드 (Codex 버전에 따라 필드가 가감될 수 있음) */
...
export class Bridge extends EventEmitter {
  ...
  private async handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    if (req.headers['x-orangi-token'] !== this.token) {
